...
def build_kv_cache_scheme(mode: str):
    """
    FP8은 compressed-tensors kv_cache_scheme으로 저장할 수 있다.
    FP16은 KV cache 양자화가 아니라 추론 엔진의 캐시 dtype 선택이므로
    여기서는 None을 반환하고, 추론 시점에 dtype으로 제어한다.
    """
    mode = mode.lower()
...
# 기본 문장 + QA 방어 + IFEval 기반 instruction-following 샘플 통합
calibration_texts = [
...
] * 16 # 총 샘플 수 조절 (총 304개), 반복된 문장을 통해 안정적인 분포 추정
...
model = AutoModelForCausalLM.from_pretrained(
    MODEL_ID,
    torch_dtype=torch.bfloat16, #양자화 전 기준 정밀도
    device_map="auto",
